...
// Bit-ops: delegate to smsd:: primitives from bitops.hpp
using smsd::popcount64;
using smsd::ctz64;
using smsd::popcountAnd;
using smsd::popcountN;
...
public:
          ...
          availBuf_(((gt.n + 63) >> 6), 0)
    {
        ...
        if (!gpuDomainOk && Nq_ > 0 && Nt_ > 50) {
            // Flat-bucket approach: two-pass to avoid hash-map overhead (Phase 3.1)
            // Pass 1: compute label keys per target atom, find maxKey
            std::vector<int> tLabelKeys(Nt_);
            int maxKey = 0;
            for (int j = 0; j < Nt_; ++j) {
            ...
            // Count entries per label key
            std::vector<int> bucketCount(maxKey + 1, 0);
            for (int j = 0; j < Nt_; ++j) ++bucketCount[tLabelKeys[j]];
            // Prefix-sum for bucket start offsets
            std::vector<int> bucketStart(maxKey + 2, 0);
            for (int k = 0; k <= maxKey; ++k)
                bucketStart[k + 1] = bucketStart[k] + bucketCount[k];
            // Pass 2: scatter into flat data array
            std::vector<int> flatBucket(Nt_);
            std::vector<int> writePos(bucketStart.begin(), bucketStart.begin() + maxKey + 1);
            for (int j = 0; j < Nt_; ++j)
                flatBucket[writePos[tLabelKeys[j]]++] = j;
            // Iterate flat buckets for domain init
            for (int i = 0; i < Nq_; ++i) {
                for (int k = 0; k <= maxKey; ++k) {
            ...
            while (changed && acIters < kMaxACIters) {
                ...
                for (int qi = 0; qi < Nq_; ++qi) {
                    for (int nk = 0; nk < gq_.degree[qi]; ++nk) {
                        ...
                        // Fast bit-parallel pre-check: if adjLong[tj] and
                        // domain(qk) share no bits, support is impossible.
                        for (int w = 0; w < tWords_; ++w) {
                            ...
                            while (bits) {
                                ...
                                // O(n/64) pre-check: any neighbor of tj in domain(qk)?
                                if (anyBitAnd(gt_.adjLong[tj].data(), domRow(qk), tWords_)) {
                                    // Confirmed overlap; verify bond compatibility
    ...
    int domainSupport(int qi) const {
        return popcountN(domRow(qi), tWords_);
    }
        ...
        for (int pos = 0; pos < Nq_; ++pos) {
            ...
            if (activeComp >= 0) {
                for (int i = 0; i < Nq_; ++i) {
                    ...
                    int size = popcountN(&workDomain[i * tWords_], tWords_);
