...
public:
        ...
        : g1_(g1), g2_(g2), C_(C), induced_(induced)
    {
        ...
        for (int i = 0; i < n1; ++i) {
            for (int j = 0; j < n2; ++j)
                if (atomsCompatFast(g1, i, g2, j, C)) compatTargets_[i].push_back(j);
        }
    }
    ...
    std::map<int,int> seedExtendMCS(TimeBudget& tb, int upperBound) {
        ...
        // Use precomputed compatibility from GraphBuilder (v6.5.3 perf fix)
    ...
    static void mcSplitRecurse(
        ...
        if (!branchFromTarget) {
            ...
            for (int tj = tSets[bestClass].nextSetBit(0); tj >= 0; tj = tSets[bestClass].nextSetBit(tj + 1)) {
                ...
                if (!qiHasMappedNb && !triedOrbits.insert(g2.orbit[tj]).second) continue;
            ...
            for (int qi = qSets[bestClass].nextSetBit(0); qi >= 0; qi = qSets[bestClass].nextSetBit(qi + 1)) {
                nodeCount++;
                if (nodeCount > MAX_NODE_LIMIT || tb.expired()) return;
                if (!tjHasMappedNb && !triedOrbits.insert(g1.orbit[qi]).second) continue;
...
inline int orientationSeedProbeSize(const MolGraph& query, const MolGraph& target,
    ...
    return static_cast<int>(gb.seedExtendMCS(tb, upperBound).size());
}
...
inline int orientationMcSplitProbeSize(const MolGraph& query, const MolGraph& target,
    ...
    TimeBudget tb(24LL * 60 * 60 * 1000);
    int64_t nodeCount = 0;
    return static_cast<int>(gb.mcSplitSeed(tb, nodeCount).size());
}
...
inline std::map<int,int> findMCSImpl(const MolGraph& g1, const MolGraph& g2,
    ...
    int mcSplitSize;
    {
        ...
        if (weightMode ? mcScore > bestScore : isBetterMCS(g1, mcSeed, static_cast<int>(mcSeed.size()), best, bestSize, bestHetero)) {
...
inline McsResult findMCSFromSmiles(const std::string& smi1, const std::string& smi2,
    ...
    int minN = std::min(g1.n, g2.n);
    result.tanimoto = minN > 0 ? static_cast<double>(result.size) / minN : 0.0;
...
/// Rarity tier weight for an atomic number. Tier 3 (S,P,Se,B,Si)=3.0,
