...
public final class SearchEngine {
  ...
  private static Map<Integer, Integer> repairInvalidMcsMapping(
    ...
    List<String> errors = validateMapping(g1, g2, mapping, C);
    if (errors.isEmpty()) return mapping;

    String err = errors.get(0);
    List<Integer> candidates = new ArrayList<>(2);
    java.util.function.IntConsumer addCandidate =
        qi -> {
          if (qi < 0 || candidates.contains(qi)) return;
          candidates.add(qi);
        };
    java.util.function.IntUnaryOperator parseInt =
        pos -> {
          int end = pos;
          if (end < err.length() && err.charAt(end) == '-') end++;
          while (end < err.length() && Character.isDigit(err.charAt(end))) end++;
          return Integer.parseInt(err.substring(pos, end));
        };

    if (err.startsWith("atom mismatch: q")) {
      int qPos = err.indexOf('q');
      if (qPos >= 0) addCandidate.accept(parseInt.applyAsInt(qPos + 1));
    } else if (err.startsWith("bond missing: q(") || err.startsWith("bond incompatible: q(")) {
      int qStart = err.indexOf('(');
      int dash = err.indexOf('-', qStart);
      if (qStart >= 0 && dash >= 0) {
        addCandidate.accept(parseInt.applyAsInt(qStart + 1));
        addCandidate.accept(parseInt.applyAsInt(dash + 1));
      }
    }
    if (candidates.isEmpty()) addCandidate.accept(mapping.keySet().iterator().next());

    Map<Integer, Integer> best = Collections.emptyMap();
    for (int qi : candidates) {
      Map<Integer, Integer> reduced = new LinkedHashMap<>(mapping);
      reduced.remove(qi);
      Map<Integer, Integer> repaired = repairInvalidMcsMapping(g1, g2, reduced, C);
      if (repaired.size() > best.size()) best = repaired;
    }
    return best;
  }
